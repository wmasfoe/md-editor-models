...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune Qwen2.5 on Markdown/Writing assistance dataset with LoRA")
    ...
    parser.add_argument("--output_dir", type=str, default="output/qwen-0.5b-writing-lora", help="Directory to save LoRA checkpoints")
    ...
    parser.add_argument("--merged_output_dir", type=str, default="output/qwen-0.5b-writing-merged", help="Directory to save the merged standalone model")
...
def main():
    ...
    print("=" * 60)
    print(f"🚀 Starting SFT Fine-Tuning Pipeline for Writing Assistant")
    ...
    sft_config = SFTConfig(
        ...
        dataset_text_field=None,  # 自动使用 messages 对话字段
        report_to="none"  # 不强制上传 wandb
    )
    ...
    if args.merge_and_save:
        print("\n🔄 Merging LoRA adapter with base model for standalone export...")
        # 重新以全精度加载基座以避免量化合并精度损失
        ...
        print(f"👉 You can now convert it to GGUF using llama.cpp!")

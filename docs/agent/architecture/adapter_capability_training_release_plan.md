...
## 10.1 数据集重建验证（2026-09-03）

已在本机构建并写入当前训练数据：

```text
去重前：54,550 条
去重后：47,290 条（移除 7,260 条）
训练集：42,561 条（精确重复 303 条，受每文本最多 10 条上限约束）
验证集：4,729 条（精确重复 3 条）
训练/验证完整样本交集：0 条
```

当前任务分布（训练集）：GEC 25,443、Completion 13,468、Distill 3,600、Preserve 50。完整样本已按分组切分，避免同一模板同时落入训练和验证；文档来源 `source_id` 级切分仍是后续数据管道改进项。
